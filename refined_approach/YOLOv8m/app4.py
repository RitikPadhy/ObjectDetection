...
# Global set the seen objects
seen_objects_global = set() 
...
def filter_and_count(results, sec, motion_track):
    ...
    for box in results.boxes:
        ...
        object_signature = (label, round(center[0], -1), round(center[1], -1))  # Rounded center for tolerance

        if object_signature in seen_objects_global:
            ...
            if prev_obj["label"] == label:
                ...
                if dist < 50:
                    if is_object_moving_toward(prev_obj["area"], area):
                        ...
                        seen_objects_global.add(object_signature)  # Add to seen set
                    else:
        ...
        if not matched and sec == 0:
            counts[label] = counts.get(label, 0) + 1
            motion_track.append({"label": label, "status": "towards", "center": center})
            seen_objects_global.add(object_signature)
